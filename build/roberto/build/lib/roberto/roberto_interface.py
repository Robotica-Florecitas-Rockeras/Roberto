...
class RobertoInterfaceNode(Node):

    def __init__(self, update_signal, update_orientation_signal, updated_vel_signal, update_simTime_signal):

        super().__init__("roberto_interface")
        self.get_logger().info('Roberto Interface Started\n')
        ...
        self.roberto_position = self.create_subscription(Twist, "/roberto_position", self.update_roberto_position, 10)
        self.roberto_orientation = self.create_subscription(Float32, "/roberto_orientation", self.update_roberto_orientation, 10)
        self.roberto_vel = self.create_subscription(Twist, "/roberto_cmdVel", self.update_roberto_vel, 10)
        self.roberto_simTime = self.create_subscription(Float32, "/simulationTime", self.update_roberto_simTime, 10)



    def call_player_service(self, file_path):

        client = self.create_client(UpdateFilename, '/roberto_player_service')
    ...
    def update_roberto_position(self, msg: Twist):
    ...
    def update_roberto_orientation(self, msg:Float32):
    ...
    def update_roberto_vel(self, msg: Twist):
    ...
    def update_roberto_simTime(self, msg:Float32):
...
class RosThread(QThread):
    ...
    def run(self):
        rclpy.init()
        self.node = RobertoInterfaceNode(self.position_updated, self.orientation_updated, self.vel_updated, self.simTime_updated)  
...
class MainUX(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Roberto Position")
        self.move(480, 200)
        ...
        # Tortuguita :)
        self.roberto_label = QLabel(self)
        self.roberto_pixmap = QPixmap("/home/robotica/Desktop/Taller1/src/roberto_controller/roberto_controller/roberto.png")  

        if self.roberto_pixmap.isNull():
            print("Error: No se pudo cargar la imagen de la tortuga")

        self.roberto_label.setPixmap(self.roberto_pixmap)
        self.roberto_label.setScaledContents(True)
        self.roberto_label.setFixedSize(50, 50)  # Tamaño de la tortuga

        # Instanciar Tortuguita 
        layout.addWidget(self.roberto_label)
        ...
        self.ros_thread.position_updated.connect(self.update_roberto_position)  # Conexión de la posición entre ROS y la interfaz
        self.ros_thread.orientation_updated.connect(self.update_roberto_orientation)
...
# Guardar trayectoria de roberto en un archivo .json
...
# Guardar gráfigo .png de la trayectoria del roberto
    ...
    def update_roberto_position(self, position_data):
        ...
        self.roberto_label.move(screen_x - roberto_size // 2, screen_y - roberto_size // 2)
    

# Actualización de la trayectoria de roberto en la interfaz
    
    def update_roberto_orientation(self, theta):
        self.current_theta = theta  # Guarda el ángulo
        self.update_roberto_rotation()


    def update_roberto_rotation(self):
        transform = QTransform()
        transform.rotate(-(abs(self.current_theta) * 180 / 3.1416) + 90)
        rotated_pixmap = self.roberto_pixmap.transformed(transform, Qt.SmoothTransformation)
        self.roberto_label.setPixmap(rotated_pixmap)
